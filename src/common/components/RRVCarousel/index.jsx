...
export const RRVCarousel = (props) => {
  ...
  const showButton = (isNext = true) => {
    if (cycleNavigation) return true;

    const last = Array.isArray(children) ? children.length - 1 : 0;

    if (isNext && state.active === last) return false;
    if (!isNext && state.active === 0) return false;
  ...
  return (
    ...
    >
      <StyledItemWrapper style={{ height: height || childrenHeight }}>
