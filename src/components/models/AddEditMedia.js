...
import { addNewMedia, editSong, getAlbums } from "redux/actions";
...
export const AddEditMedia = ({
  ...
  const { album, media, filePath, songEdit } = useSelector((state) => state);
  const { albums } = album;
  const { mediaAdding, mediaAdded } = media;
  const { hasUploaded, filePathName } = filePath;
  const { loading: updating, loaded: updated } = songEdit;
  ...
  useEffect(() => {
    if (hasUploaded) {
      setNewMedia((prev) => ({ ...prev, mediaLink: filePathName }));
    }
  }, [hasUploaded, filePathName]);
  useEffect(() => {
    if (currentMedia) {
      const { title, album, type, mediaLink, author, actionDate } =
        currentMedia;
      setNewMedia({
        title,
        albumId: album.id,
      ...
      <Card>
        ...
        <Card.Body>
          ...
          {currentMedia ? (
            ...
            >
              {updating ? "Updating,..." : `Update ${currentMedia.title}`}
            </Button>
          ) : (
            <Button
